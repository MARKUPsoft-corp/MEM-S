...
import { ContentService, DEFAULT_HERO_SLIDES, type HeroSlide } from '~~/services/contentService'
...
const slides = ref<Slide[]>([...DEFAULT_HERO_SLIDES])
...
onMounted(async () => {
    try {
        const loadedSlides = await ContentService.getHeroSlides()
        if (loadedSlides && loadedSlides.length > 0) {
            slides.value = loadedSlides
        }
    } catch (err) {
        console.warn('[HeroSection] Erreur chargement slides dynamiques:', err)
    }
    startAutoPlay()
})
