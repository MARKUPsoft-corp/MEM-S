<template>
  <div class="babouches-page">
    <!-- Hero Banner -->
    <section class="page-hero" :style="heroBannerStyle">
      <div class="hero-overlay"></div>
      <div class="hero-content">
        <h1 class="hero-title">Collection Babouches</h1>
        <p class="hero-subtitle">Élégance traditionnelle</p>
      </div>
    </section>
...
const allProducts = computed(() => {
  return productsStore.products
    .filter(p => ['babouches-cuir', 'babouches-brodees'].includes(getCategorySlug(p)) || getCollectionSlug(p) === 'babouches')
    .map((product: any) => ({
      id: product.id,
      name: product.name,
      slug: product.slug,
...
const overlayProducts = computed(() => {
  ...
  return allProducts.value.filter(p => getCategorySlug(p) === targetSlug)
})
...
const cuirProducts = computed(() => allProducts.value.filter(p => getCategorySlug(p) === 'babouches-cuir'))
const brodesProducts = computed(() => allProducts.value.filter(p => getCategorySlug(p) === 'babouches-brodees'))
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Découvrez notre collection de babouches artisanales en cuir : élégance traditionnelle et confort moderne.'
    }
  ]
})
</script>
...
.page-hero {
  ...
  width: 100%;
  height: 100vh;
  background-image: url('https://images.unsplash.com/photo-1543163521-1bf539c55dd2?w=1920&h=600&fit=crop&q=80');
